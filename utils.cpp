...
/* returns a pixel value from a 32-bit floating point image */
//inline
unsigned char pixval8c(IplImage* img, int r, int c)
{
...
/* sets a pixel value in a 64-bit floating point image */
//inline
void setpix8c(IplImage* img, int r, int c, float val)
{
...
char* prepend_path(const char* path, const char* file)
{
  ...
  sprintf( pathname, "%s/%s", path, file );//snprintf( pathname, name_length, "%s/%s", path, file );
...
{
  const char* last_slash = strrchr( pathname, '/' );  // last / in pathname
